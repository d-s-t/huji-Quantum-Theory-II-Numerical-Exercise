...
\begin{figure}[h!]
    ...
    \end{minipage}%
\end{figure}
קבוע ההתכנסות ממהתאמה הוא \input{results/first_order/q_nm_o1} לעומת \input{results/first_order/q_nm} ללא התיקון, כלומר יש שיפור משמעותי בקצב ההתכנסות.
...
נשאר רק להסתכל על התוצאות. 
\begin{figure}[h!]
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \begin{tabular}{c c c}
            ללא תיקון & תיקון מסדר ראשון & תיקון מסדר שני \\
            \hline
            \input{results/first_order/q_nm} & \input{results/first_order/q_nm_o1} & \input{results/second_order/q_nm_o2} \\
        \end{tabular}
        \captionof{table}{.}
        \label{tab:second_order_ground_energy}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{plots/second_order/residual_error.eps}
        \caption{השגיאה השיורית עבור תיקון מסדר שני}
